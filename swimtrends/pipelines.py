...
class MeetResultsPGPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        ra_SQL = "INSERT INTO race(ra_nbr,ra_title,ra_status,ra_gender,ra_is_para,ra_distance,ra_stroke,ra_relay_count,ra_link,meet_id) SELECT %s,%s,%s,%s,%s,%s,%s,%s,%s,%s WHERE NOT EXISTS (SELECT 1 FROM race WHERE meet_id=%s AND ra_nbr=%s AND ra_status=%s) RETURNING race_id;"
        re_SQL = "INSERT INTO race_result(re_swimmer,re_swimmer_details,re_birth,re_nationality,re_is_para,re_team,re_rank,re_points,re_points_calc,re_points_fixed,re_completed_time,race_id) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT ON CONSTRAINT unique_swimmer_result DO NOTHING;"
        ...
        for race in item['race']:
            ra_data = (race['nbr'],race['title'],race['session'],race['gender'],race['is_para'],race['distance'],race['stroke'],race['relay_count'],race['page'],item['meetId'],item['meetId'],race['nbr'],race['session'])
            ...
            for result in race['results']:
                logging.debug("Inserting race result: %s", result)
                re_data = (result['swimmer'],result['swimmer_url'],result['year_of_birth'],result['nationality'],result['is_para'],result['team'],result['rank'],result['points'],result['points_calc'],result['points_fixed'],result['completed_time'],race_id)
